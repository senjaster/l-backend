...
PUT /aggregate?force=false - create or replace aggregate. The following rules apply
    - No aggregate {id} in path. It is passed in body only
    - Logical deletion via is_deleted=True works at aggreagate and at children level
    - If force=false (default value): 
      - If aggreagate already exists, then "server_modified_at" should match value in the database. Reject otherwise with 409 error. Provide error message with conflict explanation
      - ignore server_modified_at for new instances
      - if server_modified_at match and there are "extra" child entities (matched by id) on server, reject 409 and throw error. Provide error message with conflict explanation
    - if force=true:
    ...
    - Put should never "steal" child entities from other aggreagate. By "stealing" I mean changing parent_entity_id of existing child row. Even force=True does not allow that
