...
class EquipmentTypeRepository:
    ...
    async def get_all(
        ...
        equipment_types = [
            row
            async for row in queries.get_all_equipment_types(
        ...
        templates_raw: list[dict] = [
            row async for row in queries.get_all_control_point_templates(conn)
        ]
        templates_by_type = {
        ...
        return [
            EquipmentType(
                **row, control_point_templates=templates_by_type.get(row["id"], [])
            )
    ...
    async def get_by_id(self, conn, equipment_type_id: int) -> Optional[EquipmentType]:
        ...
        template_rows = [
            row
            async for row in queries.get_control_point_templates(
                conn, equipment_type_id=equipment_type_id
            )
        ]
    ...
    async def save(self, conn, equipment_type: EquipmentType) -> EquipmentType:
        ...
        await queries.upsert_equipment_type(
            conn, id=equipment_type.id, name=equipment_type.name
        )

        # Synchronize control point templates
        await self._sync_templates(
            conn, equipment_type.id, equipment_type.control_point_templates
        )
    ...
    async def _sync_templates(
        self, conn, equipment_type_id: int, templates: list[ControlPointTemplate]
    ):
        """Synchronize control point templates: match by ID, add new, delete removed"""
        # Get existing template IDs
        existing_rows = [
            row
            async for row in queries.get_template_ids(
                conn, equipment_type_id=equipment_type_id
            )
        ]
