...
class GroupRepository:
    ...
    async def delete(
        ...
        existing = await queries.get_by_id(conn, id=group_id)
        if not existing:
            return False
        
        if hard_delete:
            # Hard delete - cascades to group_plant via FK
            result = await queries.delete_group_hard(conn, id=group_id)
        else:
            # Soft delete
            result = await queries.delete_group(conn, id=group_id)
        
        # Also soft delete all subgroups recursively
        if not hard_delete:
            await queries.soft_delete_subgroups(conn, parent_id=group_id)
    ...
    async def move_group(
        self, conn, group_id: UUID, new_parent_id: Optional[UUID]
    ) -> Group:
        """
        ...
        Raises:
            ValueError: If validation fails (group not found, cyclic dependency, etc.)
        """
        current = await self.get_by_id(conn, group_id, include_children=False)
        if not current:
            raise ValueError(f"Group {group_id} not found")
        ...
        if new_parent_id:
            ...
            parent = await queries.get_by_id_active(conn, id=new_parent_id)
            if not parent:
                raise ValueError(f"Parent group {new_parent_id} not found")
            
            result = await queries.check_cyclic_dependency(
                conn, group_id=group_id, new_parent_id=new_parent_id
            )
            
            if result and result.get("would_create_cycle", False):
                raise ValueError("Cyclic dependency detected")
            
        await queries.move_group(conn, group_id=group_id, new_parent_id=new_parent_id)
        
        updated = await self.get_by_id(conn, group_id, include_children=False)
        if not updated:
            raise ValueError(f"Group {group_id} not found after move")
