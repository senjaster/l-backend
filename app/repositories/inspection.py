...
from app.models.inspection import (
    Inspection, InspectionStep, ImageLink,
    InspectionListItem, InspectionListResponse
)
...
class InspectionRepository:
    ...
    def _build_inspection_aggregates(
        ...
        for row in step_rows:
            ...
            step = InspectionStep(
                **step_data,
                image_links=image_links_by_step.get(step_id, [])
            )
        ...
        for inspection_row in inspection_rows:
            ...
            inspection = Inspection(
                **inspection_row,
                steps=steps_by_inspection.get(inspection_id, [])
            )
    ...
    async def get_by_plant_id(self, conn, plant_id: UUID, modified_since: datetime = DEFAULT_MODIFIED_SINCE) -> list[Inspection]:
        ...
        inspection_rows = [row async for row in queries.get_by_plant_id(conn, plant_id=plant_id, modified_since=modified_since)]
    ...
    async def save(self, conn, inspection: Inspection, force: bool = False) -> Inspection:
        ...
        if current and not force:
            ...
            if truncate_to_milliseconds(inspection.server_modified_at) != truncate_to_milliseconds(current.server_modified_at):
                raise ConcurrentModificationError(
                    ConflictError(
